...
Date = date.toLocaleString();  const noteTitle = document.getElementById("noteTitle");

      const title = version.title || "Untitled Note";
  ...
  function showAbout() {
    showToast("Notepad Pro — rich-text notepad with title, autosave and version history.", "info");
  }
  ...
  function getTitle() {
    return noteTitle.value.trim() || "Untitled Note";
  }
  ...
  async function copyNote() {
    try {
      await navigator.clipboard.writeText(`${getTitle()}\n\n${note.innerText}`);
